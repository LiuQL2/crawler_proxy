...
import os
import sys
from hdf_proxy.configuration.parameters import DATABASE_INFO as database_info
from hdf_proxy.configuration.parameters import PROXY_TABLE as proxy_talbe
...
class DatabaseOperationClass(object):
    def __init__(self):
        """
        类的初始化
        :return:无返回数据
    ...
    def select(self, table, record = None, database = None):
        ...
        try:
            ...
            if database == None:
                ...
                pass
            if record == None:
                if table == proxy_talbe['name']:
                    sql = 'select * from '+ database + '.' + table + ' where ' + proxy_talbe['proxy_status'] +' = %s'
                    print sql,proxy_talbe['proxy_useful']
                    cursor.execute(sql, [proxy_talbe['proxy_useful']])
    ...
    def __get_column_name__(self, table, database):
        """
        用于获得表的列名
...
if __name__ == '__main__':
    db = DatabaseOperationClass()
    # data = db.select(table = 'proxy', record=dict(proxy_ip='23.106.159.52',proxy_status = "True"))
    # data = db.select(table = 'proxy_effective')
    # db.insert(table = 'proxy_effective',record={'proxy_type':'HTT', 'proxy_ip':'12.13', 'proxy_port':'3302'})
    # db.insert(table = 'circle',database='xunyiwenyao',record={'circle_id':'Liuqianlong', 'circle_name':'刘乾龙', 'circle_intro':'3302'})
    # db.delete(table = 'proxy_effective', record={'proxy_ip':'12.13', 'proxy_port':'3302'})
    # db.delete(table = 'circle', database='xunyiwenyao',record={'circle_id':'Liuqianlong', 'circle_name':'杨薇'})
    # db.update(table='proxy_effective', record = {'proxy_type':'hp', 'proxy_port':'33'}, primary_key={'proxy_ip':'12.13', 'proxy_port':'3302'})
    # db.update(table='circle', database='xunyiwenyao',record = {'circle_name':'', 'circle_intro':'刘乾龙','circle_suffer_number':'32'}, primary_key={'circle_id':'Liuqianlong', 'circle_name':'刘乾龙'})
    data = db.select(table = 'circle',database='xunyiwenyao')
    # data = db.select(table = 'proxy_effective')
    # db.delete(table='proxy_effective',record = {'proxy_ip':'12.13'})
    print type(data)
    print data
